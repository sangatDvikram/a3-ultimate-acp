...
$(document).ready(function(){
 $('[rel="tooltip"]').tooltip('toggle');
	...
	history.pushState(stateObj, "page 2", 'http://'+host+'/Eshop/'+pushUrl);
	}
			checkURL(pushUrl);
			return false;
	});
...
function checkURL(hash)
{
	...
	newurl=newurl.replace("http://"+host+"/Eshop/","");
...
function loadPage(url1)
{
	...
	var n=url1.split("=");
	if($.inArray(n[0], filters)=='-1'){window.location='http://'+host+'/404.php';}else{
	...
	$.ajax({
		type: "POST",
		url: "http://"+host+"/Stats/eshop-view.php",
		data: datastring,
		dataType: 'json',
...
item='item='+item;
	...
	{
	var stateObj = { stats: this.hash };
	history.pushState(stateObj, "page 2", 'http://'+host+'/Eshop/'+item);
	}
	loadPage(item);
}
