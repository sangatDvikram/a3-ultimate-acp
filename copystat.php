...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>A3 Ultimate - Coppy Stats</title>
...
if ($rec1 == 1)
	{
		...
		$copy1="SELECT c_headera FROM charac0 WHERE c_id = '$data[fullname]'";
		$kRs1 = odbc_exec($con,$kSql1);
		$check_woonz = odbc_result($kRs1,'pcoins');
		$copy2 = odbc_exec($con,$copy1);
		$copy = odbc_result($copy2,'c_headera');
		...
		if($check_woonz >= 0 )
		{
			...
			$newString = $copy;
			if(empty($err)) {
			$sqlgo= "UPDATE charac0 SET c_headera = '$newString' WHERE c_id = '$char'";
			...
			if(!$rs12)
				...
				else
				{
					...
					$msg[] = "Successfully copied all stats!.";
...
<center>Costs 0 Premium Coins </center><br/>Currently you have <?php echo $fgh1['pcoins']; ?> Premium Coins in your account and after using this service you will have <?php echo $fgh1['pcoins'] - 0; ?> Premium coins left in your account.<br><br><br>
   <label class="control-label" for="inputEmail">Char Name to Copy From</label>
    <div class="controls">
   <input type="text" id="fullname" name="fullname" class="k-textbox" style="color:#000;" placeholder="Full name" <?php if(isset($data['fullname'])) { echo 'value="'.$data['fullname'].'"'; } ?> required validationMessage="Please enter Full Name"  data-provide="typeahead" data-items="10" data-source='[""<?php $sqlstring1="select top 10 * from charac0 ";
...
echo ",\"".trim($dd['c_id'])."\"";
    ...
    <div class="controls">
   <input type="text" id="character1" name="character1" class="k-textbox" style="color:#000;" placeholder="Char name" required validationMessage="Please enter char Name"  data-provide="typeahead" data-items="10" data-source='[""<?php $sqlstring1="select top 10 * from charac0 ";
$r1= odbc_exec($con,$sqlstring1);
while($dd = odbc_fetch_array($r1)) {
echo ",\"".trim($dd['c_id'])."\"";
} ?>]'/>
